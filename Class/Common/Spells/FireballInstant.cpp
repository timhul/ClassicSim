...
FireballInstant::FireballInstant(Character* pchar_,
    ...
    Spell(QString("Fireball%1").arg(name_),
...
void FireballInstant::spell_effect() {
    ...
    const unsigned damage_dealt = instant_dmg->get_roll()
                                  + static_cast<unsigned>(round(pchar->get_stats()->get_spell_damage(MagicSchool::Fire) * spell_coefficient));
