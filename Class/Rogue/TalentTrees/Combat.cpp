    ...
    spells(static_cast<RogueSpells*>(pchar->get_spells()))
{
    ...
    QMap<QString, Talent*> tier2 {{"2ML", new Talent(pchar, this, "Deflection", "2ML", "Assets/ability/Ability_parry.png", 5, "Increases your Parry chance by %1%.", QVector<QPair<unsigned, unsigned>>{{1, 1}})},
                                  {"2MR", new Precision(pchar, this)}};
    add_improved_backstab(tier2);
    add_talents(tier2);

    QMap<QString, Talent*> tier3 {{"3ML", new Talent(pchar, this, "Riposte", "3ML", "Assets/ability/Ability_warrior_challange.png", 1, "A strike that becomes active after parrying an opponent's attack. This deals 150% weapon damage and disarms the target for 6 sec.", QVector<QPair<unsigned, unsigned>>{})},
