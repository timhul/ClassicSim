...
void JudgementOfTheCrusader::buff_effect_when_applied() {
    pchar->get_target()->get_stats()->increase_spell_damage_vs_school(static_cast<unsigned>(round((holy_damage_base + pvp_glove_bonus + libram_of_fervor_bonus) * improved_sotc_mod)),
                                                                      MagicSchool::Holy);
}

void JudgementOfTheCrusader::buff_effect_when_removed() {
    pchar->get_target()->get_stats()->decrease_spell_damage_vs_school(static_cast<unsigned>(round((holy_damage_base + pvp_glove_bonus + libram_of_fervor_bonus) * improved_sotc_mod)),
                                                                      MagicSchool::Holy);
}
