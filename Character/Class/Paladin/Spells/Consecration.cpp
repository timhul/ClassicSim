...
void Consecration::calculate_damage() {
    ...
    const double damage_dealt = ((full_duration_dmg + bonus_from_spell_dmg) / 4 + tick_rest) * resist_mod;
...
void Consecration::enable_spell_effect() {
    buff->enable_buff();
}

void Consecration::disable_spell_effect() {
    buff->disable_buff();
}
