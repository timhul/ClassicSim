...
NatureDamageAttack::NatureDamageAttack(Character* pchar, const QString &name, const unsigned min, const unsigned max) :
    Spell(name, pchar, false, 0, 0),
    min(min), max(max),
    random(new Random(min, max))
{}

NatureDamageAttack::~NatureDamageAttack() {
    delete random;
}

void NatureDamageAttack::spell_effect() {
