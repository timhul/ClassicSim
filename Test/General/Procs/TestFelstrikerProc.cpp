...
void TestFelstrikerProc::test_crit_applied_and_removed() {
    ...
    FelstrikerBuff* buff = dynamic_cast<FelstrikerBuff*>(pchar->get_active_buffs()->use_shared_buff("Felstriker"));
    ...
    const double crit_before_buff = pchar->get_stats()->get_mh_crit_chance();
    buff->apply_buff();
