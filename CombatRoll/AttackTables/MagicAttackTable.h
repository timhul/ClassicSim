...
public:
    MagicAttackTable(Mechanics* mechanics, Random* random, const int clvl, const double spell_hit, const int target_res);
    ~MagicAttackTable() = default;

    int get_hit_outcome(const unsigned roll, const double crit_chance) const;
    int get_resist_outcome(const unsigned roll) const;
    void update_miss_chance(const int clvl, const double spell_hit);
